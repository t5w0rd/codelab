...
async def go_buttons(controller_state: ControllerState):
    """
    Example controller script.
    Navigates to the "Test Controller Buttons" menu and presses all buttons.
    """
    ...
    # push all buttons consecutively until user input
    loop = ['l', 0.2, 'a', 0.2, 'a', 0.6, 'a', 0.2]
    #while not user_input.done():

    while True:
        await ainput(prompt='Press any key to continue...')
        ...
        for _ in range(48):
            ...
            for e in loop:
                if isinstance(e, str):
                    await button_push(controller_state, e, sec=0.1)
...
def _register_commands_with_controller_state(controller_state, cli):
    ...
    async def go():
        ...
        """
        await go_buttons(controller_state)

    cli.add_command(go.__name__, go)
